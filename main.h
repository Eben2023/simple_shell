...
/**
 * struct data_struct - contains arguments to pass into a function,
 * allowing
 * uniform prototype for function pointer struct
 *
 * @arg: a string generated from the standard input stream containing
 * the arguments
 * @argv: an array of strings generated from @arg
 * @path: a string representing the path to the current command
 * @argc: an integer representing the number of arguments passed to the
 * command
 ...
 * @linecount_flag: a boolean flag indicating whether the line count is
 * on or off
 ...
 * @env_changed: a boolean flag indicating whether the environment variables
 * were changed
 * @status: an integer representing the return status of the last executed
 * command
 * @cmd_buf: the address of a pointer to cmd_buf, indicating whether command
 * chaining is on
 * @cmd_buf_type: an integer representing the type of command chaining
 * (CMD_type ||, &&, ;)
 * @readfd: an integer representing the file descriptor from which to read
 * the input line
